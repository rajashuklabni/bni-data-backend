  ...
  try {
    ...
    await db.query('UPDATE transactions SET einvoice_generated = $1 WHERE order_id = $2', [true, order_id]);
      ...
      return res.status(200).json({
        message: hasNoGstin ? "Document generated successfully (no IRN, Visitor GSTIN not provided)" : "Document generated successfully (no IRN, GSTIN is N/A)",
        data: {
          ackNo: null,
          ackDate: null,
          irn: null,
...
if (req.body.transactionId?.payment_time) {
  ...
  paymentTime = `${dateStr} ${timeStr}`;
    ...
    const mailOptions = {
      ...
      attachments: [
        {
          filename: `${req.body.orderId.payment_note === 'visitor-payment' || req.body.orderId.payment_note === 'Visitor Payment' ? req.body.orderId.visitor_name : memberName} - ${docNo}.pdf`,
          path: pdfPath
        }
      ]
    };
